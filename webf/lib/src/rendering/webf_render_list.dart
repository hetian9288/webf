...
/// add some sliver optimization
/// 1. First Paint Layout child optimization
class WebFRenderSliverList extends RenderSliverMultiBoxAdaptor {
  bool _finishFRLayout = false;
  ...
  void performLayout() {
    ...
    if (firstChild == null) {
      ...
      if(!_finishFRLayout) {
        _finishFRLayout = true;
        if(targetEndScrollOffset > constraints.viewportMainAxisExtent) {
          targetEndScrollOffset = constraints.viewportMainAxisExtent;
        }
      }
    }
    ...
    earliestScrollOffset = childScrollOffset(earliestUsefulChild)!) {
      ...
      if (earliestUsefulChild == null) {
        ...
        if (scrollOffset == 0.0) {
          ...
          print('layout sliver item use time o: ${DateTime.now().microsecondsSinceEpoch - record3.microsecondsSinceEpoch}');
          earliestUsefulChild = firstChild;
    ...
    if (leadingChildWithLayout == null) {
      ...
      // print('layout sliver item use time f: ${DateTime.now().microsecondsSinceEpoch - record3.microsecondsSinceEpoch}');
      leadingChildWithLayout = earliestUsefulChild;
      trailingChildWithLayout = earliestUsefulChild;
    }
    ...
    bool advance() { // returns true if we advanced, false if we have no more children
      ...
      if (!inLayoutRange) {
        if (child == null || indexOf(child!) != index) {
          ...
          RenderBox? realChild = (child as RenderSliverRepaintProxy).child;
          // Layout the child.
